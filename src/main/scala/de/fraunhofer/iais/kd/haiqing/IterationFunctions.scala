...
class ModelConst(val window: Int, val vectorSize: Int, val maxNumSenses: Int,
                 ...
                 val modelValidateIter: Int, val maxEmbNorm: Float, val senseProbThresh: Float) extends Serializable {
  val MAX_EXP = 6
...
class ModelUpdater(val m: ModelConst, seed: Long, sy0: Array[Array[Array[Float]]], sy1: Array[Array[Array[Float]]]) {
  ...
  def adjustSentence(sentence: Array[Int], sentenceNEG: Array[Array[Int]]): (Boolean) = {
    ...
    for (pos <- 0 until sentence.size) {
      ...
      if (m.numberOfSensesPerWord(word) > 1) {
        ...
        if (word * m.ENCODE + bestSense != sentence(pos)) {
          adjusted = true
        }
        sentence(pos) = word * m.ENCODE + bestSense
      }
    }
    (adjusted)
  }
      ...
      for (p <- pos - m.window to pos + m.window) {
        if (p != pos && p >= 0 && p < sentence.size) {
          ...
          if (sentenceNEG != null) {
            ...
            for (neg <- NEG) {
              loss += -math.log(1 - actFunSigmoidApprox(emb(syn0, inpWs), emb(syn1, neg)))
              lossNum += 1
            }
          }
        }
      ...
      for (p <- pos - m.window to pos + m.window) {
        if (p != pos && p >= 0 && p < sentence.size) {
          ...
          for (i <- 0 until m.vectorSize) {
            ...
            embInc(syn0, inpWs, i, -alpha * (derivX * emb(syn1, sentence(p))(i)).toFloat)
          }

          for (j <- 0 until negs.length) {
            val negWord = negs(j) / m.ENCODE
            val negSense = negs(j) % m.ENCODE
            for (i <- 0 until m.vectorSize) {
              embInc(syn1, negs(j), i, -alpha * (derivY(j) * emb(syn0, inpWs)(i)).toFloat)
              embInc(syn0, inpWs, i, -alpha * (derivY(j) * syn1(negWord)(negSense)(i)).toFloat)
            }
          }
          lossNum += 1
        }
  ...
  /**
    ...
    * @return
    */
  def getLossDerivLogSoftmax(inpWs: Int, pos: Int, sentence: Array[Int], sentenceNEG: Array[Array[Int]], exact: Boolean):
  (Double, Array[Float]) = {
    val synInp = syn0
    val synOut = syn1
    val debug = false
